...
public class ModelTrainer {
    ...
    public static void main(String[] args) {
        ...
        SparkSession spark = SparkSession.builder().appName(APP_NAME).master().config("spark.executor.memory", "2147480000").config("spark.driver.memory", "2147480000").config("spark.testing.memory", "2147480000").getOrCreate();
        ...
        boolean exists = trainFile.exists();
        if(exists){
    ...
    public void logisticRegression(SparkSession spark) {
        ...
        Pipeline mdl1 = new Pipeline();
        mdl1.setStages(new PipelineStage[]{logReg});

        PipelineModel model1 = mdl1.fit(lblFeatureDf);
